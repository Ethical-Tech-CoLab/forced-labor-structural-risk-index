...
The Forced Labor Structural Risk Index (FLSRI) — a composite index published as a static site, with Python scripts that fetch the underlying indicators from World Bank and EPR sources.
...
Ten CDN `<script>` tags are currently loaded without a Subresource Integrity attribute. A fix is pending in **PR #3**.
